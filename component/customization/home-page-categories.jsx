...
export const HomePageCategories = ({configuration, setConfiguration }) => {
  

  const { sortCategories, HomeCategorySizeAndType } = configuration;
  const [multiSelected, setMultiSelected] = useState("");
  const [sizeAndTypeCategories, setSizeAndTypeCategories] = useState(
    HomeCategorySizeAndType
  );
  ...
  const CategoriesOptions = HomeCategorySizeAndType.map(
  ...
  useEffect(() => {
    ...
    setConfiguration({
      ...configuration,HomeCategorySizeAndType : sizeAndTypeCategories,
    });
  ...
  const handleConfirm = async () => {
    ...
    if (lat > 1) {
      console.log("Latest Videos Selected Twice...");
      setMultiSelected("Latest Videos");
    }
    if (originals > 1) {
    ...
    if (pop, lat, free, originals, Favourites === 1) {
      try {
        ...
        const putConfig = await axios.put(
      ...
      console.log('some values are false')
    }
    setLoading(false);
  };
    ...
    >
      ...
      <Grid container>
        {sizeAndTypeCategories.map((item, sizeAndTypeIndex) => {
          ...
          if (sizeAndTypeIndex + 1 === 3) iterationText = "3rd";
          if (sizeAndTypeIndex + 1 > 3){
            iterationText = `${sizeAndTypeIndex+1}th`;
          }

          // console.log(item);
          return (
            <Grid
              key={sizeAndTypeIndex}
              item
              xs={12}
              md={6}
              lg={4}
              sx={{ margin: "21px 0", minHeight: "80px" }}
            >
              <Typography sx={{ fontSize: "21px" }}>
                {iterationText} Category
              </Typography>
              <Typography
                sx={{ width: "90%", margin: "12px 0" ,color : '#6082B6',fontSize:'20px',fontWeight:'700'}}
                >
                {item.categoryName}
              </Typography>
              <Select
                ...
                sx={{
                  ...
                  margin: "8px 0",
                  border:
                    multiSelected === item.categoryName
                      ? "3px solid red"
                      : "none",
                }}
                disabled={true}
